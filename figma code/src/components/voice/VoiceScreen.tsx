...
import { Mic, Square, Sun, Moon, ArrowLeft, AlignRight, MessageCircle, X } from 'lucide-react'
    ...
    >
      ...
      <div className="flex flex-col items-center pb-14 z-10 relative">
        <button
          ...
          onClick={handleMic}
          disabled={!supported}
          aria-label={isCallActive ? 'إنهاء المكالمة' : 'بدء المكالمة'}
          className="relative flex h-16 w-16 items-center justify-center rounded-full disabled:opacity-40 group"
        >
          <span className="gradient-border relative flex h-full w-full items-center justify-center rounded-full text-white shadow-[0_0_20px_rgba(168,85,247,0.4)] transition-all transition-transform group-hover:scale-105 group-hover:shadow-[0_0_30px_rgba(168,85,247,0.6)]">
            {isCallActive ? (

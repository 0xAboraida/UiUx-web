  ...
  return (
    <div className="flex flex-col items-center px-4 pb-8 z-10 relative mt-auto w-full">
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-3">
        ...
        >
          {/* Top Bar above Input: Domain Selector */}
          <div className="flex items-center justify-start px-1" ref={dropdownRef}>
            <div className="relative">
              <div className="gradient-border rounded-[20px] p-[2px] shadow-lg shadow-primary/20 transition-all duration-300">
                <button
                  type="button"
                  onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                  className={`group flex items-center gap-2 px-3.5 h-[38px] sm:h-[42px] rounded-[18px] transition-all duration-300 ${dark
                      ? 'bg-[#1a0730]/90 backdrop-blur-md text-white/90 hover:bg-[#1a0730]'
                      : 'bg-white/95 backdrop-blur-md text-brand-deep hover:bg-white'
                    }`}
                >
                  <div className={`flex items-center justify-center p-1 rounded-lg transition-colors ${dark ? 'bg-brand-magenta/20 text-brand-magenta' : 'bg-primary/10 text-primary'
                    }`}>
                    {selectedDomain.isZadIcon ? (
                      <img src={zadDarkLogo} alt="Zad" className="h-4 w-4 object-contain opacity-90 drop-shadow-md" />
                    ) : (
                      selectedDomain.icon
                    )}
                  </div>
                  <span className="font-sans font-bold text-xs sm:text-sm tracking-wide">
                    {selectedDomain.id === 0 ? 'المجال: تلقائي' : `المجال: ${selectedDomain.name}`}
                  </span>
                  <ChevronUp className={`w-3.5 h-3.5 mr-0.5 transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : 'rotate-0 opacity-70'}`} />
                </button>
              </div>

              {/* Glowing Dropdown Menu */}
              <div className={`absolute bottom-full right-0 mb-2 w-[220px] rounded-2xl border backdrop-blur-2xl transition-all duration-300 z-50 origin-bottom-right ${isDropdownOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-2 pointer-events-none'
                } ${dark
                ...
                <div className="p-2.5">
                  <div className={`mb-2 px-2 text-xs text-center font-bold font-sans ${dark ? 'text-white/80 border-b border-white/10 pb-2' : 'text-brand-deep/80 border-b border-black/10 pb-2'}`}>
                    ...
                    {DOMAINS.map(d => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-2.5 p-2 rounded-xl transition-all duration-200 text-right group ${isActive
                              ? dark
                                ? 'bg-gradient-to-r from-brand-magenta/20 to-purple-500/10 text-white border border-brand-magenta/40 shadow-sm'
                              ...
                              : dark
                                ...
                                : 'hover:bg-black/5 text-brand-deep/80 border border-transparent'
                            }`}
                        >
                          <div className={`flex items-center justify-center p-1.5 rounded-lg transition-all ${isActive
                              ? (dark ? 'bg-brand-magenta/30 text-white' : 'bg-primary/20 text-primary')
                              : (dark ? 'bg-white/5 text-brand-magenta' : 'bg-black/5 text-primary')
                            }`}>
                            {d.isZadIcon ? (
                              <img src={zadDarkLogo} alt="Zad" className="h-4 w-4 object-contain drop-shadow-sm" />
                            ) : (
                              d.icon
                            )}
                          </div>
                          <span className={`text-xs font-sans font-bold ${isActive ? '' : (dark ? 'group-hover:text-white' : 'group-hover:text-primary')}`}>
                            {d.name}
                          </span>
                        </button>
                      )
          ...
          {/* Bottom Row: Main Input Bar + Voice Button */}
          <div className="flex w-full items-end gap-2 sm:gap-3">
            {/* Main Input Bar */}
            <div className="gradient-border min-w-0 flex-1 rounded-[22px] sm:rounded-[24px] p-[2px] sm:p-[2.5px] shadow-xl shadow-primary/25 transition-all duration-300">
              <div
                className={`flex items-end gap-2 rounded-[20px] sm:rounded-[22px] py-1.5 sm:py-2 pr-3.5 sm:pr-5 pl-2 transition-all duration-300 ${dark ? 'bg-[#1a0730]/90 backdrop-blur-md' : 'bg-white/95 backdrop-blur-md'
                  }`}
              >
                <VoiceRecordButton
                  listening={listening}
                ...
                <TextareaAutosize
                  ...
                  className={`font-sans min-w-0 flex-1 resize-none bg-transparent py-2 text-sm sm:text-base outline-none placeholder:text-current/50 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] ${dark ? 'text-white placeholder:text-white/50' : 'text-brand-deep placeholder:text-brand-deep/45'
                    }`}
                />
                <button
                  type="submit"
            ...
            {/* Voice-chat button */}
            <VoiceChatButton onClick={onOpenVoice} className="shrink-0 h-[50px] w-[50px] sm:h-[60px] sm:w-[60px]" />
          </div>
        </form>
      </div>
    </div>
